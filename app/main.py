...
from src.portfolio_app.ui import inject_global_css_banner_and_disclaimer, apply_background_image, set_sidebar_width
from src.portfolio_app.data import get_price_data, daily_returns
from src.portfolio_app.optimizers import (
    optimize_mv, optimize_target_vol, optimize_min_cvar, optimize_hrp, HAS_CVXPY, HAS_RISKFOLIO
)
from src.portfolio_app.metrics import sample_frontier, port_metrics, hist_var_cvar, mc_stats_only
...
if res is None:
    ...
    st.markdown(
        """
        <div style="background: white; padding: 4px 12px; margin: 36px 0 12px 0; text-align: center;
                    font-size: 2.0rem; font-weight: bold; color: black; box-shadow: 0 2px 6px rgba(0,0,0,0.06);">
    ...
    with cL:
        ...
        st.markdown(f"""<div class="custom-table" style="background-color: white; padding: 0; border-radius: 0px;
                    box-shadow: 0 2px 8px rgba(0,0,0,0.10); font-size: 1rem; margin-bottom: 16px;margin-top: 4px;">
    ...
    with cR:
        ...
        st.markdown("""<style>.custom-table table { width: 100%; border-collapse: collapse; }
                    ...
                    unsafe_allow_html=True)
        st.markdown(f"""<div class="custom-table" style="background-color: white; padding: 0; border-radius: 0px;
                    box-shadow: 0 2px 8px rgba(0,0,0,0.10); font-size: 1rem; margin-bottom: 8px;margin-top: 4px;">
